...
## M11 hard-save candidate

The manually accepted but repository-unapproved `review/m10-safety-audit` working candidate adds a deterministic 25-endpoint/21-oscillator qualification fixture, a real-browser 25-position journey, repeated cleanup/rebuild cycles, sustained digital 10×/100× overload tests, stereo max-channel peak coverage, reset acknowledgement, MessagePort lifecycle, worklet fail-closed, invalid-control, active-deletion, and resource-balance checks. M09 remains authoritative until clean-runtime qualification and promotion both succeed.

Current verified local results are 78/78 Node tests and 18/18 Playwright tests. Run `pnpm run typecheck`, `pnpm run lint`, `pnpm run build`, `node --experimental-strip-types --test tests/*.test.mjs`, and `pnpm exec playwright test`. The browser suite uses a production server on `http://localhost:3002` and includes the axe serious/critical accessibility scan.

Manual Safari acceptance passed for the native-fence layered-monitor configuration, including Threads, transport, channel deletion, Sound Desk live plotting and plot changes. Open `http://localhost:3002/?audioDiagnostic=native-fence-scope` for that exact local candidate. The custom AudioWorklet master-meter path remains preserved but is not the accepted listening path because it caused progressive Safari latency. Never reproduce synthetic overload fixtures through physical monitors. The remote repository baseline remains Milestone 09 until a separate explicit promotion instruction.
